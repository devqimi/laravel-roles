...
export default function CrfForm() {
    ...
    function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        if (fileType === "application/pdf") {
            ...
            reader.onload = (ev: ProgressEvent<FileReader>) => setFilePreview(ev.target?.result as string);
        ...
        <div className="bg-white p-6 rounded shadow-md w-full max-w-lg mx-auto mt-7">
            ...
            <form className="space-y-3">
                <div>
                    ...
                    <input className="w-full border border-blue-900 rounded px-2 py-1" type="text" name="name" required value={user?.name || ""} readOnly/>
                </div>
                <div>
                    <label className="block mb-1">NRIC</label>
                    <input className="w-full border border-blue-900 rounded px-2 py-1" type="text" name="nric" required value={user?.nric || ""} readOnly/>
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="category_id" className="w-full border border-blue-900 rounded px-2 py-1" required>
                        <option value="">Select</option>
                            {categories.map((cat) => (
